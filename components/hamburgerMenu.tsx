...
const HamburgerMenu: React.FC<HamburgerMenuProps> = ({ links }) => {
  ...
  return (
    <div className='md:hidden top-5 right-5 fixed w-60 z-[999] flex flex-col items-end gap-2'>
        <motion.button variants={menuTrigger} initial="visible" whileTap="tap" whileHover="hover"
        ...
        <AnimatePresence>
          {isOpen && 
            <motion.div variants={menuList} 
            initial="start" animate="visible" 
             className='w-full bg-white drop-shadow border border-slate-400 dark:border-white border-opacity-60 shadow-2xl rounded-2xl flex flex-col items-cemter justify-center dark:bg-gray-950 p-1 '>
            {links.map((link, index)=>(
              <motion.div className='w-full' key={link.hash} initial={{y:-100, opacity:0}} animate={{y:0, opacity:1}}> 
                <NextLink className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 tranistion dark:text-gray-500 dark:hover:text-gray-300 cursor-pointer",
                  {
